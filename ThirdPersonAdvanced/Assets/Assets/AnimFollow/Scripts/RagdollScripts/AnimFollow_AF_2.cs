...
{
	public class AnimFollow_AF_2 : MonoBehaviour
	{
...
#region
...
#if RAGDOLLCONTROL
		RagdollControl_AF_2 ragdollControl;
#endif
	...
	//		Debug.Log("The script AnimFollow has set the fixedDeltaTime to " + fixedDeltaTime); // Remove this line if you don't need the "heads up"
			...
			{
				UnityEngine.Debug.LogWarning("Missing script SimpleFootIK_AF_2 on " + master.name + ".\nAdd it or comment out the directive from top line in the AnimFollow script." + "\n");
				userNeedsToAssignStuff = true;
			}
#else
			else if (master.GetComponent<SimpleFootIK_AF_2>())
			...
			#if RAGDOLLCONTROL
			if (!(ragdollControl = GetComponent<RagdollControl_AF_2>()))
			{
